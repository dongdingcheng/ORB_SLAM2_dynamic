...
public:
    ...
    void GrabStereo(const sensor_msgs::ImageConstPtr& msgLeft,const sensor_msgs::ImageConstPtr& msgRight,const sensor_msgs::ImageConstPtr& msgDepth);
...
//	Init();
    ...
    ros::NodeHandle nh;
    message_filters::Subscriber<sensor_msgs::Image> left_sub(nh, "/multisense/left/image_rect_color", 1);
    message_filters::Subscriber<sensor_msgs::Image> right_sub(nh, "/multisense/right/image_rect", 1);
    ...
    message_filters::Synchronizer<sync_pol> sync(sync_pol(10), left_sub,right_sub,depth_sub);
...
void ImageGrabber::GrabStereo(const sensor_msgs::ImageConstPtr& msgLeft,const sensor_msgs::ImageConstPtr& msgRight,const sensor_msgs::ImageConstPtr& msgDepth)
{
    ...
    if(do_rectify)
    {
        ...
        mpSLAM->TrackStereo(imLeft,imRight,cv_ptrDepth->image,cv_ptrLeft->header.stamp.toSec());
    }
    else
    {
        mpSLAM->TrackStereo(cv_ptrLeft->image,cv_ptrRight->image,cv_ptrDepth->image,cv_ptrLeft->header.stamp.toSec());
    }
