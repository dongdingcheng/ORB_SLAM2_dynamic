 ...
 */
	bool Tracking::TrackWithMotionModel()
	{
        ...
        bool isCheckDynamic= true;
        if(isCheckDynamic){

            box bb;
            mpDetectDynamic->insertData(mImGray);
            mpDetectDynamic->detectMotion(bb);
            //select Features
            if(bb.x>0&&bb.y>0){
                for(size_t i=0;i<mCurrentFrame.N;i++)
                {
                    auto &pt=mCurrentFrame.mvKeysUn[i];
                    if(pt.pt.x>bb.x&&(pt.pt.x<bb.x+bb.width)&&pt.pt.y>bb.y&&(pt.pt.y<bb.y+bb.height)){
                        auto  &pp=mCurrentFrame.mvpMapPoints[i];
                        if(pp!=nullptr&&!pp->isBad())
                            pp->SetBadFlag();
