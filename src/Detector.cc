...
// 这里的 两个文件地址需要传入
  ...
  det_net_ptr->load_param("/home/divine/ORB_SLAM2_dynamic/Thirdparty/ncnn/example/model/mobilenetv2_ssdlite_voc.param");
  det_net_ptr->load_model("/home/divine/ORB_SLAM2_dynamic/Thirdparty/ncnn/example/model/mobilenetv2_ssdlite_voc.bin");
...
//        std::cout << obj.rect.x <<"1111 "<< obj.rect.y << "222222 "<< obj.rect.height << obj.rect.width << std::endl;

        cv::rectangle(image, obj.rect, cv::Scalar(255, 0, 0));// 矩形框
//        cv::rectangle(image, cv::Rect(obj.rect.x, obj.rect.y, obj.rect.width, obj.rect.height), cv::Scalar(255, 0, 0));// 矩形框
        char text[256];
